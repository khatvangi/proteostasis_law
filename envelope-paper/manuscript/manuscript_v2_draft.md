...
> **Provenance of this file.** This is the v2 restructuring supplied 2026-07-30:
> a continuous-prose, journal-voiced rewrite of `MANUSCRIPT.md` with the Result
> numbering replaced by descriptive headings and the tables renumbered after the
> two excluded analyses were dropped. It is retained verbatim as the drafting
> source. The assembled paper — this text plus embedded figures, inline tables,
> and the excluded-analyses disclosure — is `MANUSCRIPT.md`.
...
The interaction is positive wherever it is defined and never subadditive, so the prediction is directionally confirmed. Its magnitude depends sharply on the starting margin, which is why the evaluation point governs the answer: the excess over additivity for a 3-fold error increase crossed with a 3-fold rescue knockdown is 0.2% at a margin of 2.20 log₁₀, 0.6% at 1.90, and 7.4% at *E. coli*'s own margin of 1.39. The largest excess among combinations that remain viable is 9.6%, for a 2-fold error increase with a 5-fold knockdown.
...
The framework's distinguishing prediction remains untested experimentally. Until it is tested this is a motivated constraint on extant translation rather than an established one. The margin analysis says where to look, and says that the experiment does not require a sensitized background.

---

## Materials and Methods

All analyses are reproduced by the scripts in `scripts/`, run in numeric order, from the raw inputs in `data/raw/`. Outputs are written to `data/computed/` and asserted by the accompanying test suite.
...
Raw inputs, analysis scripts, computed outputs, figures, tables, and the numeric test suite are in the accompanying repository, and a single script regenerates all of it and then runs the tests. [Deposition DOI to be assigned.]
...
## Table and figure inventory for this draft

Main tables, renumbered because two tables reporting excluded analyses are dropped and the former Table 6 is retired:

| New | Content | Was |
|---|---|---|
| Table 1 | burden terms and their flux operationalization | Table 1 |
| Table 2 | bounds on the tolerable per-codon error rate, and derived statistics | Table 2, 2b |
| Table 3 | headroom against chaperone availability θ | Table 7 |
| Table 4 | supraadditivity against starting margin | Table 5 |
| Table 5 | permutation tests on the operational axes | Table 3 |
| Table S1 | per-codon (μ, ν) coordinates | Table S1 |
| Table S2 | per-amino-acid operational spread | Table S2 |
| Table S3 | validation of the ν axis | Table S3 |
| Table S4 | supraadditivity effect-size grid at the operating point | Table S5 |
| Table S5 | the two capacity knobs are not equivalent | Table S6 |
| Table S6 | what the axis tests could have detected | Table S7 |
| Table S7 | leave-one-codon-out jackknife on the μ axis | Table S8 |

Dropped: the metal-binding-site background comparison and the cross-species Δ comparison (former Tables 4 and S4), and the retired evaluation-point-by-anchoring grid (former Table 6 / S9), which Table 3 supersedes. The renumbering has to be applied in `08_make_tables.py` and in the TSV filenames before the test suite will pass against this draft.
