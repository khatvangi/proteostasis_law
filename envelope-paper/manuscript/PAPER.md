...
That benchmark transfers a uniform shrinkage, and μ's effect is not uniform. Per-amino-acid tightness on the error axis runs from 0.06 to 2.03 in units of the spread chance predicts (median 0.52), and two of the eighteen amino acids are more spread than chance. So we imposed μ's own per-amino-acid pattern on ν, amino acid by amino acid, and re-ran the test. It is detected: Δ = 0.631 against a null of 0.884, z = −2.75, p = 0.0034. The transfer is faithful — applied to a structureless axis the same pattern gives z = −3.39 ± 0.63, against μ's own −3.54 — so ν's coarse resolution attenuates the effect, from −3.39 to −2.75, without erasing it. Under a subset model in which the shrinkage instead hits a random half of the amino acids, power over 400 replicates is 0.51 (95% CI 0.46 to 0.56) at a 60% tightening and 0.83 (0.79 to 0.86) at 80%. What these data leave open is therefore a ν effect substantially weaker than μ's, or confined to fewer amino acids, rather than a counterpart of μ's structure in the form μ actually takes.

Two properties of the μ result bound its interpretation. It is not carried by a few codons. A leave-one-codon-out jackknife leaves the clustering significant in 59 of 59 deletions, with z between −3.86 and −2.62 (Table S7); dropping CCC, the 2.0 × 10⁻² maximum that sets the 613-fold span by itself and gives proline the largest Δ_A of any amino acid, leaves z = −3.67, marginally stronger than the full set. Nor is it an artifact of mass-spectrometry detectability, which is the more serious objection and one that can be tested rather than merely noted. Per-codon μ is a mean over detected substitutions, and detection probability varies with amino acid through ionization efficiency, mass-shift resolvability, and peptide abundance, which would manufacture amino-acid-level structure with no biological content. The exposure is real: per-codon sampling depth ranges from 1 to 16 detected substitutions (median 10), depth is inversely related to μ (Spearman ρ = −0.37, p = 0.004), and depth carries as much amino-acid-level structure as μ itself (η² = 0.560 between amino acids for depth against 0.556 for log μ). The sign of that relationship identifies what it is. Error-driven detection — more errors at a codon, more substitutions seen there — predicts a positive correlation. A negative one is the signature of thin-sampling inflation, where a codon observed in few substitutions has its mean pulled up by whichever few were observed. Sampling depth is therefore a nuisance regressor rather than a mediator, and can be removed. Regressing log μ on log depth (slope −0.93, p = 6.0 × 10⁻⁴) removes the 18.8% of the variance in log μ that depth explains; on the residuals the clustering persists, Δ = 0.777 against a null of 1.130, z = −2.97, p = 0.0026, and 80% of the between-amino-acid variance survives (η² = 0.446 against 0.556). Consistently, dropping the most thinly sampled quartile of codons also leaves it intact (z = −2.85, p = 0.007). What remains unexcluded is a component of detectability that sampling depth does not track — an amino-acid-specific bias in the measured rate rather than in the number of substitutions seen — and that would need per-codon error rates from a method whose detection probability does not vary with amino acid identity.

The contrast between the axes is the more durable observation. Whatever organizes μ, and it is not sampling depth, does not produce a counterpart on ν.
...
**Fig. 2. The measured error axis is organized at the amino-acid level; the supply axis is not.** Observed operational spread Δ of synonymous codons (red line) against 10,000 within-degeneracy permutation nulls (grey; dashed line, null mean). (a) Error axis μ: synonyms cluster far more tightly than null (z = −3.56, p = 0.0010). (b) Supply axis ν: no comparable structure (z = −0.06). The test's detection floor on this axis is 19.6% below the null mean against an observed μ effect of 37.4%, and μ's own per-amino-acid pattern, imposed on ν, is detected (z = −2.75, p = 0.0034), so the absence is not a resolution artifact. (c) Per-codon log μ by amino acid, ordered by amino-acid mean; red ticks mark amino-acid means. Most of the variation is between amino acids rather than within them (η² = 0.56).
...
One bookkeeping note, because the same test appears at three precisions. The null is estimated by sampling, so z for the μ within-degeneracy test shifts in the second decimal with the number of permutations: −3.56 at 10,000 (the headline value, and the figure of record), −3.55 at the 4,000 used for the 59-fold jackknife, and −3.54 at the 2,000 used for the pattern-transfer calibration. These are the same statistic computed at different Monte Carlo precision, not three results. Where two analyses use the same count they agree exactly: the headline test and the minimum-detectable-effect confirmation both give −3.5643673490070 at 10,000 permutations.
...
To size what each axis test could have detected, within-amino-acid deviations were shrunk toward the amino-acid mean by a factor s, with s = 1 the observed code and s = 0 making all synonyms identical, and the permutation test re-run at each s (2,000 permutations per point, confirmed at 10,000 at the selected point). The minimum detectable effect is the largest s still rejected at α = 0.05, expressed as the percentage by which Δ falls below its null mean, which makes the two axes comparable. A uniform shrinkage is the wrong stand-in for μ's effect, which is strongly non-uniform across amino acids, so we also transferred μ's observed pattern directly. For each amino acid A we took `s_A`, the observed root-mean-square within-A deviation of standardized log μ divided by the deviation expected under random assignment, `sd·sqrt(1 − 1/n_A)`, so that `s_A = 1` means A's synonyms are exactly as spread as chance predicts. Those per-amino-acid factors were then applied to ν and the permutation test re-run at 10,000 permutations. As a calibration, the same factors applied to a structureless axis (log μ values shuffled across codons, 20 draws) should recover something close to μ's own z if the transfer is faithful.

Because Δ is a deterministic function of the code, a power calculation requires a stochastic effect model; we applied the shrinkage to a random subset of amino acids, each included with probability 0.5, and report the rejection rate over 400 replicates per point with a Wilson 95% interval. Each replicate is seeded from its own index rather than from a generator advanced in a loop, so the estimate does not depend on execution order and the replicates run in parallel. This subset model is a stated assumption, not a measurement of tAI error.
...
Exposure accounting of that kind cannot say whether the clustering survives removal of the depth-related component of μ, so we also residualized. Log μ was regressed on log sampling depth by ordinary least squares, and the residuals were standardized and carried through the identical pipeline: Δ, the within-degeneracy permutation test at 10,000 permutations, and the between-amino-acid variance partition. Residualizing on depth is legitimate only if depth is a nuisance regressor rather than a mediator of the biology. The sign of the relationship settles that: error-driven detection predicts more substitutions at higher-error codons, hence a positive slope, whereas the observed slope is negative, which is what thin-sampling inflation of a mean over few observations produces. We report the residualized test as the substantive check and the exposure accounting as context for it.
