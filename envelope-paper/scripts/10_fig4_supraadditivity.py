...
def main():
    ...
    ax.set_title("Additive at wild-type margin;\nsynergy appears as it closes")
    ...
    collapsed = s2.pivot_table(index="error_factor", columns="capacity_factor",
                               values="collapsed_both").sort_index(ascending=False)
    ...
    for i, e in enumerate(piv.index):
        for j, c in enumerate(piv.columns):
            if bool(collapsed.loc[e, c]):
                ax.add_patch(plt.Rectangle((j, i), 1, 1, facecolor="#cfcfcf",
                                           edgecolor="white", lw=0.4, zorder=3))
                ax.text(j + 0.5, i + 0.5, "coll.", ha="center", va="center",
                        fontsize=5.0, color="#444", zorder=4)
    ax.set_xlabel("rescue throughput knocked down (÷)")
    ax.set_ylabel("error rate raised (×)")
    ax.set_title("At the observed rate the\ninteraction stays under ~13%")
