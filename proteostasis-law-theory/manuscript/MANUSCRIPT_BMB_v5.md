...
**The old-pole aggregate load.** Section 8.3 turns the measured lineage growth difference into a requirement on the aggregate a dividing *E. coli* old-pole cell must carry, indexed by the share `β` held in the visible focus. It is the one prediction here that a single pair of measurements can refute, and the requirement sits 62× to 214× below the only aggregate load anyone has measured, so the two quantities needed are the wild-type aggregate fraction under unstressed growth and the focal share in the same cells.
