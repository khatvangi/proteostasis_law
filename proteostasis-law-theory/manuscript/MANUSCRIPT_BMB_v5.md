...
Integration confirms the instability independently of the branch computation. Perturbing each of the 104 crossing networks at its own `tr J` maximum, all 104 grow by more than tenfold and 102 escape, with median `max|δ|/δ₀` of 9851; 205 non-crossing networks perturbed at their own `tr J` maximum give 0 escapes and median 1.00, with no overlap between the distributions (Fig. 4b). All 309 states are exact equilibria. The integration also recovers the eigenvalue: growth exponent within 5% of `max Re(λ)` in 90 of 104, and the period of `‖δ‖` within 5% of `π/ω` in 47 of 49. Split by group the escape rates are indistinguishable — 60 of 61 against 42 of 43 — and that is a limit of the test rather than a result. The escape threshold is 10⁻² of the state scale against a 10⁻⁶ perturbation, so escape records a ten-thousandfold amplification, which growth onto a stable limit cycle produces exactly as reliably as divergence does. The escape statistic measures departure from a small neighbourhood and cannot distinguish the two.
